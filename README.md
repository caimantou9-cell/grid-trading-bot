...
```bash
# 1. 克隆代码
git clone https://github.com/caimantou9-cell/grid-trading-bot.git
cd grid-trading-bot
...
cd ~/grid-trading-bot
# 确保开头有 (.venv) 字样，如果没有，执行下面的激活命令：
source .venv/bin/activate

# 下载代码
git clone https://github.com/x10xchange/python_sdk.git

# 安装它（注意这里用的是你刚下载的真实文件夹名）
...
# 后台
nohup python3 main.py --config config.json &

```
常用命令2：

/root/grid-trading-bot

screen -dmS gridbot bash -c 'source .venv/bin/activate && python main.py --config config.json --log-level DEBUG'

查看运行状态: screen -r gridbot
退出查看（不停止）: 按 Ctrl+A 然后按 D
查看日志: tail -f /opt/grid-bot/grid.log
停止机器人: screen -S gridbot -X quit
